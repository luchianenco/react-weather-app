...
import Details5Days from './details';
...
class LocationDetailed extends React.Component {
    ...
    componentDidMount() {
        this.props.loadForecast(this.props.match.params.location, constant.LOCATION_FORECAST_DURATION_5DAYS);
    }

    renderForecast() {
        const prefix = 'fcst_5d_' + this.props.match.params.duration;
        return this.props.forecast.data.list.map((forecast, index) => {
            return <Details5Days key={prefix + index} details={forecast}/>
        })
    }

    renderDurationLink() {
        return <Link to={`/weather/${this.props.match.params.location}/16days`}>16 Days</Link>
    }
...
export default withRouter(
    connect(
        state => ({
            forecast: state.location.forecast5days
        }),
        dispatch => ({
